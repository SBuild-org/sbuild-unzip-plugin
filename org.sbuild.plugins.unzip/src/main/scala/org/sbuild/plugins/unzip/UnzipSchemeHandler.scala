...
import org.sbuild.plugins.unzip.internal.Util
 ...
 *
 */
class UnzipSchemeHandler(val _baseDir: File = null, regexCacheable: Boolean = false)(implicit project: Project)
    extends SchemeResolver
    with SchemeResolverWithDependencies
    with CacheableSchemeResolver {

  private[this] def log = Logger[UnzipSchemeHandler]
        ...
        config match {
          case config @ FileConfig(fileInArchive, archiveFile, file) =>
            ...
            if (!file.exists || file.lastModified < zipFile.lastModified) try {
              Util.unzip(zipFile, file.getParentFile, List((fileInArchive -> file)), project.monitor)
              try {
...
${e.getMessage}""", e)
          ...
          case RegexConfig(archive, regex, extractDir) =>
            ...
            val extractedFiles = Util.unzip(zipFile, extractDir, List(), project.monitor, Some { name: String => pattern.matcher(name).matches() })
